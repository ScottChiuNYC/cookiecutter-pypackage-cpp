...
* pybind11 is currently fetched from the pybind11 repo on GitHub. It was added by vcpkg from the registry but the dependencies might fail to download if you are behind a corporate corporate firewall. The fetch-from-repo trick works well with header-only libraries but for compiled libraries the setup will be more complicated
